...
describe('v3 board tests', () => {
  ...
  test('v3 board methods', async () => {
    ...
    var response = await test_board.get();
    ...
    var iterator = await test_board.get_pins();
    expect('test_iterator').toEqual(iterator);
    expect(mock_get_iterator.mock.calls[0][0]).toEqual('/v3/boards/test_board_id/pins/');

    iterator = await test_board.get_sections();
    expect('test_iterator').toEqual(iterator);
    expect(mock_get_iterator.mock.calls[1][0]).toEqual('/v3/board/test_board_id/sections/');
