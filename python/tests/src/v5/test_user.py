...
class UserTest(unittest.TestCase):
    ...
    def test_user_get_boards(self, mock_api_object_init, mock_api_object_get_iterator):
        ...
        response = test_user.get_boards('test_user_data', query_parameters={
            'param1': 'value1',
            'param2': 'value2'})
        mock_api_object_get_iterator.assert_called_once_with('/v5/boards?param1=value1&param2=value2')
        ...
        response = test_user.get_boards('test_user_data', query_parameters={'param1': 'value1',})
    ...
    def test_user_get_pins(self, mock_api_object_get_iterator):
        ...
        class BoardMock:
            def __init__(self, board_id, api_config, api_access_token):
                self.id = board_id
                assert api_access_token == 'test_access_token', 'access_token not as expected'
        ...
        mock_api_config.api_uri = 'test_uri' # just to make underlying ApiObject init work
        ...
        with mock.patch('src.v5.user.Board', BoardMock):
            for index, pin in enumerate(test_user.get_pins('test_user_data',
