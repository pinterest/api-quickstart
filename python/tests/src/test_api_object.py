...
class ApiObjectTest(unittest.TestCase):
    ...
    def test_api_object_v3(self, rm):
        ...
        rm.delete(
            self.test_uri + "/delete_path",
            request_headers=access_token_header,
            status_code=403,
            reason="test deletion failure",
            text="should be ignored",
        )
        # verify check called after delete
        with self.assertRaisesRegex(
            RuntimeError, "request failed with reason: test deletion failure"
        ):
    ...
    def test_api_object_iterator(self, rm):
        ...
        rm.get(
            ...
            json={"data": ["one", "two"], "bookmark": "BOOKMARK1"},
        )  # v3 uses data
        rm.get(
