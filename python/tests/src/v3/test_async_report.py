...
class AsyncReportTest(unittest.TestCase):
    ...
    def test_async_report(self, mock_api_object_init, mock_post_data, mock_request_data):
        test_async_report = AsyncReport('test_api_config', 'test_access_token', 'test_advertiser_id')
        mock_api_object_init.assert_called_once_with('test_api_config', 'test_access_token')
        
        with self.assertRaisesRegex(RuntimeError, 'subclass must override the kind_of report'):
        ...
        test_report = TestReport('test_api_config', 'test_access_token', 'test_advertiser_id')
        with self.assertRaisesRegex(RuntimeError, 'subclass must override post_uri_attributes()'):
        ...
        test_report2 = TestReport2('test_api_config', 'test_access_token', 'test_advertiser_id')
        ...
        mock_request_data.return_value = {'report_status': 'FINISHED', 'url': 'test_report2_url'}
    ...
    @mock.patch('src.v3.async_report.ApiObject.post_data')
    @mock.patch('src.v3.async_report.ApiObject.__init__')
    def test_async_report_run(self, mock_api_object_init, mock_post_data, mock_request_data,
        ...
        test_report3_url = 'test_report3_url/x-y-z/metrics_report.txt?Very-long-credentials-string'
        test_report3 = TestReport3('test_api_config', 'test_access_token', 'test_advertiser_id')
        ...
        mock_sleep.assert_has_calls([call(1), call(2), call(4), call(8), call(10), call(10)])

        # check calls to print()
        mock_print.assert_has_calls([call('Report status: IN_PROGRESS. Waiting a second...'),
        ...
        self.assertEqual(test_report3.filename(), 'metrics_report.txt') # verify filename()
